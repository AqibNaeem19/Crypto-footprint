...
const CryptoDetails = () => {
  ...
  // Time Period for Stats of any coin
  const time = ['3h', '24h', '7d', '30d', '1y', '3m', '3y', '5y'];

  if (isFetching) return 'Loading...';

  // Extract values from response object and apply markup
  const stats = [
    { title: 'Price to USD', value: `$ ${cryptoDetails?.price && millify(cryptoDetails?.price)}`, icon: <DollarCircleOutlined /> },
    { title: 'Rank', value: cryptoDetails?.rank, icon: <NumberOutlined /> },
    { title: '24h Volume', value:  '$ ' + (cryptoDetails?.["24hVolume"] && millify(cryptoDetails?.["24hVolume"])).toString(), icon: <ThunderboltOutlined /> },
      ...
      <Col className="coin-desc-link">
        ...
        <Col className="coin-links">
          ...
          {cryptoDetails.links.map((link) => (
            <Row className="coin-link" key={link.key}>
